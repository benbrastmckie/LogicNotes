...
%   
% NOTE: There are no problems for this section. 
%
% \begin{enumerate}[leftmargin=1.2in]
%   \item[\bf Subframe:] $\tuple{T', <'}$ is a \textit{subframe} of $\tuple{T, <}$ \textit{iff} $T' \subseteq T$ and $x < y$ whenever $x <' y$. 
%   \item[\bf History:] $\T'$ is a \textit{history} of $\T$ \textit{iff} (1) $\T'$ is a subframe of $\T$ that satisfies \textsc{tot}; and (2) for every subframe $\T''$ of $\T$ that satisfies \textsc{tot}, if $\T'$ is a subframe of $\T''$, then $\T' = \T''$ (i.e., $\T'$ is a maximal total subframe of $\T$).
%   \item[\bf Possible Histories:] Let $H_\T$ be the set of all histories of $\T$.
%   \item[\bf Inclusion:] We may take $x \in \T' \coloneq (x \in T') \wedge (\T' = \tuple{T', <'})$ for convenience. 
%   \item[\bf Stability Set:] Let $H_\T^x \coloneq \set{\T' \in H_\T \mid x \in \T'}$ be the histories of $\T$ where $x \in \T'$.
%   \item[\bf Language $\boldsymbol{\TL_\square}$:] $\TL_\square$ extend $\TL$ to include `$\Boxdot$' where $\corner{\Boxdot\metaA}$ reads $\ulcorner$It is inevitable that $\metaA\urcorner$.
%   \item[\bf Inevitability:] $\vDash$ is defined recursively for a model $\M = \tuple{T, <, \I}$ of $\TL$, history $\tuple{T', <'}$, times $x, y \in T$, sentence letter $p_i \in \SL$, and $\metaA, \metaB \in \wfs{\TL}$:
%     \begin{itemize}[leftmargin=.15in]\small
%       \item[] $\M, \tuple{T', <'}, x \vDash p_i$ \textit{iff} $x \in \I(p_i)$.
%       \item[] $\M, \tuple{T', <'}, x \vDash \neg \metaA$ \textit{iff} $\M, x\nvDash \metaA$.
%       \item[] $\M, \tuple{T', <'}, x \vDash \metaA\rightarrow \metaB$ \textit{iff} $\M, \tuple{T', <'}, x\nvDash \metaA$ or $\M, \tuple{T', <'}, x \vDash  \metaB$.
%       \item[] $\M, \tuple{T', <'}, x \vDash \Past \metaA$ \textit{iff} $\M, \tuple{T', <'}, y\vDash \metaA$ for every $y \in T'$ such that $y <' x$.
%       \item[] $\M, \tuple{T', <'}, x \vDash \Future \metaA$ \textit{iff} $\M, \tuple{T', <'}, y\vDash \metaA$ for every $y \in T'$ such that $x <' y$.
%       \item[] $\M, \tuple{T', <'}, x \vDash \Boxdot \metaA$ \textit{iff} $\M, \tuple{T'', <''}, x\vDash \metaA$ for every $\tuple{T'', <''} \in H_\T^x$.
%     \end{itemize}
%   \item[\bf Truth-Value Gaps:] Thomason defines \textit{truth} relative to a model and time as:
%     \begin{itemize}[leftmargin=.15in]\small
%       \item[] $\M, x \vDash \metaA$ \textit{iff} $\M, \T', x \vDash \metaA$ for all $\T' \in H_\T^x$.
%       \item[] $\M, x \Dashv \metaA$ \textit{iff} $\M, \T', x \vDash \metaA$ for all $\T' \in H_\T^x$.
%     \end{itemize}
