...
\begin{enumerate}
  ...
  \item[\bf Proof Lines:] A \textit{proof line} may be represented by a tuple $\tuple{n,i,\metaA,J}$ consisting of a \textit{position number} $n$, \textit{indentation number} $i$, wfs $\metaA$ of $\FCH$, and \textit{justification} $J$. % where premises are justified by `PR'.
...
\begin{enumerate}[leftmargin=1.2in,labelsep=.15in] 
  ...
  \item[\bf Hilbert System:] Consider the following \textit{axiom schemata} \textbf{A1} -- \textbf{A3} and \textit{rule schema} \textbf{MP}:
    ...
    \item[\bf Axioms:] The \textit{axioms} of the Hilbert proof system are the instances of \textbf{A1} -- \textbf{A3}.
    \item[\bf Hilbert Proof:] A \textit{Hilbert proof} of $\metaA$ from $\MetaG$ is a finite sequence $X$ of wfss of $\PL$ ending in $\metaA$ where every wfs in $X$ is either: (1) a \textit{premise} in $\MetaG$; (2) an \textit{axiom}; or (3) follows from previous lines by an instance of \textbf{MP}. 
    \item[\bf Derivable:]
      A wfs $\metaA$ of $\PL$ is \textit{derivable} (\textit{provable}) from $\MetaG$ by the Hilbert proof system, i.e., $\MetaG \PLproves \metaA$, just in case there is a Hilbert proof $X$ of $\metaA$ from $\MetaG$.
      % An $\FCHsystem$ \textit{theorem} is any wfs $\metaA$ of $\FCH$ where $\varnothing\FCHproves \metaA$. % (written $\FCHproves \metaA$).
      \begin{itemize}
      ...
      \end{itemize}
    \item[\bf Naturalness:] Whereas the Fitch system idealizes natural forms of reasoning, the Hilbert system is easier to state and study but harder to use.
\end{enumerate}
...
% TODO:
  ...
  % eliminates redundancies but at the cost of ease of use and naturalness
...
%\item[\bf Intension:] For any interpretation $\MLmodels$ of $\PL_\square$ and sentence letter $p_i$ in $\PL_\square$, we may refer to the function $\MLmodels(p_i)$ from $W$ to $\set{0,1}$ as the \textit{intension} of $p_i$.
