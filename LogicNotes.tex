...
\section*{\sc Propositional Logic: Naturalness and Metalogic}

\begin{enumerate}[leftmargin=1.2in,labelsep=.15in] 
  \item[\bf Fitch System:] The Fitch proof system has the following metalogical properties:
    \begin{itemize}
  ...
  \item[\bf Equivalent:] Two proof systems $A$ and $B$ for the same language are \textit{extensionally equivalent} just in case $\MetaG \vdash_A \metaB$ if and only if $\MetaG \vdash_B \metaB$.  
  \item[\bf Hilbert System:] The \textit{Hilbert proof system} consists of the following \textit{axiom schemata} \textbf{A1} -- \textbf{A3} and \textit{rule schema} \textbf{MP}:
    ...
    \item[\bf Derivable:]
      ...
      % An $\FCHsystem$ \textit{theorem} is any wfs $\metaA$ of $\FCH$ where $\varnothing\FCHproves \metaA$. % (written $\FCHproves \metaA$).
    \item[\bf Metalogic:] The Hilbert proof system has the following metalogical properties:
      \begin{itemize}
...
\section*{\it Problem Set: Naturalness and Metalogic}
