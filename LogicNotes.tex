...
\begin{enumerate}[leftmargin=1.2in,itemsep=2pt]
	\item[\bf Axiom Schemata:] Consider the following axiom schemata:\vspace{-.05in}
	      \begin{multicols}{2}
		      \begin{enumerate}
			      \item[\bf K] $\Box(\metaA\rightarrow \metaB)\rightarrow(\Box \metaA\rightarrow\Box \metaB)$.
...
\begin{enumerate}[leftmargin=1.2in,itemsep=2pt]
	...
	\item[\bf TK Rule:] Consider the following metarule:
	      \begin{enumerate}[leftmargin=.5in,labelsep=.15in,itemsep=.075in]
		      \item[\bf TK] If $\MetaG \vdash \metaA$, then $\Future\MetaG \vdash \Future\metaA$ (future necessitation).
	      \end{enumerate}
	      where $\Future\MetaG = \set{\Future\metaC \mid \metaC \in \MetaG}$.

	\item[\bf TL System:] The \textit{TL proof system} for linear tense logic includes all $\TL$ instances of the Hilbert axioms together with axioms \textbf{TL}, \textbf{T4}, and \textbf{TA}, along with the \textbf{TD} duality rule and \textbf{TK} future necessitation rule.

	\item[\bf Tense Proofs:] A $\TLSystem$-\textit{proof} of $\metaB$ from $\MetaG$ is a finite sequence $\metaA_1, \ldots, \metaA_n$ where $\metaA_n = \metaB$ and each $\metaA_i$ is either: (1) a \textit{premise} in $\MetaG$; (2) a propositional \textit{axiom}; (3) a \textbf{TL} \textit{axiom} instance; or (4) follows from previous formulas by modus ponens, the \textbf{TK} rule, or the \textbf{TD} duality rule.
...
%
%   \item[\bf Metalogical Consequences:] The soundness theorem has several important consequences:
%   \begin{itemize}
...
%     \item \textbf{Compactness:} If every finite subset of $\Gamma$ is satisfiable, then $\Gamma$ is satisfiable
...
%
% \begin{enumerate}[leftmargin=1.2in,itemsep=2pt]\small
%   \item[\bf Soundness Proofs:] Prove that the following axioms and rules preserve validity, contributing to the soundness theorem:
%     \begin{enumerate}[label=\arabic*.]\small
%       \item \textbf{Modus Ponens:} Show that if $\metaA$ and $\metaA \rightarrow \metaB$ are both valid, then $\metaB$ is valid. (\textit{Hint:} Assume both formulas hold at an arbitrary model, history, and time; use the truth condition for $\rightarrow$ to establish $\metaB$.)
%       \item \textbf{MK Axiom:} Show that $\Box(\metaA \rightarrow \metaB) \rightarrow (\Box\metaA \rightarrow \Box\metaB)$ is valid over all task frames. (\textit{Hint:} Assume the antecedent holds; use the semantics of $\Box$ to quantify over all histories containing a given time.)
%       \item \textbf{MF Bimodal Axiom:} Show that $\Box\Future\metaA \rightarrow \Future\Box\metaA$ is valid over all task frames. (\textit{Hint:} Show that modal necessity of a future formula implies that at all future times, the formula holds at all histories passing through those times.)
%       \item \textbf{Optional Challenge:} Choose two additional axioms from the TM system (MT, M4, MB, TD, TL, T4, TA, TF) and prove their validity over task frames.
%     \end{enumerate}
%
...
%     \end{enumerate}
%
%   \item[\bf Countermodels:] For each invalid formula, construct an explicit task model refuting it:
%     \begin{enumerate}[label=\arabic*.]\small
